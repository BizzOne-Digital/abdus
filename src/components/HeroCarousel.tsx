    ...
    >
      ...
      <div className="hero__stage container">
        <AnimatePresence initial={false} custom={direction} mode="wait">
          ...
          >
            {index === 0 && <SlideLeadership reduced={reduced} />}
            {index === 1 && <SlideMeet reduced={reduced} />}
            {index === 2 && <SlidePlan reduced={reduced} />}
          </motion.div>
        </AnimatePresence>
      </div>
...
function SlideLeadership({ reduced }: { reduced: boolean }) {
  return (
      ...
      >
        <motion.p className="hero-eyebrow hero-eyebrow--lead" variants={staggerChild}>
          <span className="hero-eyebrow__text">
            <span className="hero-eyebrow__mark">City</span> Councillor · Ward 1
          </span>
          <MapleLeaf />
        </motion.p>
        <motion.h1 className="hero-title hero-title--lead" variants={staggerChild}>
          <span className="hero-title__line">Strong</span>
          <span className="hero-title__line">Leadership.</span>
        ...
        <motion.div className="hero-actions" variants={staggerChild}>
          <Link to="/vision" className="btn btn--primary btn--pill">
            Our Priorities <span className="btn__chevron" aria-hidden="true" />
          </Link>
          <Link to="/about" className="btn btn--ghost btn--pill">
            Meet Shinwary
          </Link>
        </motion.div>
      </motion.div>
      ...
      >
        <LiquidGlass hover={false} className="hero-signature__card">
          ...
          <div className="hero-signature__text">
            <strong>Vote A. Salam Shinwary</strong>
            <span>A Strong Voice for Ward 1</span>
          </div>
        </LiquidGlass>
      </motion.aside>
    </div>
...
function SlideMeet({ reduced }: { reduced: boolean }) {
  const pills = [
    "15+ Years of Service",
...
function SlidePlan({ reduced }: { reduced: boolean }) {
  const items = [
